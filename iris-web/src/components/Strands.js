...
void main() {
  ...
  float spec = pow(max(dot(p / max(r, 1e-4), 0.0), 0.0), 6.0);
...
export default class Strands {
  constructor(container, options = {}) {
    ...
    this.glassMesh = new Mesh(this.gl, { geometry: this.geometry, program: this.glassProgram });
  ...
  update(t) {
    ...
    this.program.uniforms.uColors.value = buildPalette(current.colors);
    this.program.uniforms.uColorCount.value = Math.min(current.colors.length, MAX_COLORS);
    this.program.uniforms.uStrandCount.value = Math.min(Math.max(Math.round(current.count), 1), MAX_STRANDS);
    this.program.uniforms.uSpeed.value = current.speed;
    this.program.uniforms.uAmplitude.value = current.amplitude;
    this.program.uniforms.uWaviness.value = current.waviness;
    this.program.uniforms.uThickness.value = current.thickness;
    this.program.uniforms.uGlow.value = current.glow;
    this.program.uniforms.uTaper.value = current.taper;
    this.program.uniforms.uSpread.value = current.spread;
    this.program.uniforms.uHueShift.value = current.hueShift;
    this.program.uniforms.uIntensity.value = current.intensity;
    this.program.uniforms.uOpacity.value = current.opacity;
    this.program.uniforms.uScale.value = current.scale;
    this.program.uniforms.uSaturation.value = current.saturation;
    ...
    if (current.glass) {
      ...
      this.glassProgram.uniforms.uRefraction.value = current.refraction;
      this.glassProgram.uniforms.uDispersion.value = current.dispersion;
      this.glassProgram.uniforms.uRadius.value = 0.46 * current.glassSize;
